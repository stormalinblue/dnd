...
export function mapSkillMap<T, V>(map: SkillMap<T>, cb: (elem: T, key: SkillKey) => V): Array<V> {
  let result: Array<V> = [] 
...
export function mapAbilityMap<T, V>(map: AbilityMap<T>, cb: (elem: T, key: AbilityKey) => V): Array<V> {
  let result: Array<V> = [];
