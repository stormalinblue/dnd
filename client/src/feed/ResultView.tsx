...
function ResultCard(props: {
  ...
  const levelClass =
    props.level % 2 === 0 ? 'roller-even-level-card' : 'roller-odd-level-card';

  return (
    <div className={['roller-result-card', levelClass].join(' ')}>
      {props.children}
    </div>
  );
}

function ExprAssignLabel(props: {
  name?: string;
  rhs: string;
}): React.ReactNode {
  if (props.name) {
...
function ConstantResultView(props: {
...
function MultiDieRollResultView(props: {
  result: MultiDieRollResult;
  level: number;
}) {
  let sumStringComponents: Array<string> = props.result.dieResults.map(
    (result) => {
...
function SumResultView(props: { result: SumResult; level: number }) {
  let sumStringComponents: Array<string> = props.result.children.map(
    (result) => {
