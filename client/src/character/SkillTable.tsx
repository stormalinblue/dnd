...
import { type Character, type SkillKey, type SkillCharacteristic, SKILL_ATTRIBUTES, type AbilityKey, ABILITY_NAMES, mapSkillMap, scoreModifier } from "./character";
import { ModifierLabel, D20RollButton } from "./common";
...
function SkillTableRow(props: { character: Character, skillKey: SkillKey }) {
  ...
  const rawModifierExpression = constant(rawModifier, `${abilityName} Modifier`);
  ...
  if (skillProficient) {
    ...
    const proficiencyBonusExpression = constant(props.character.proficiencyBonus, 'Proficiency Bonus');
    expression = sum([d20(), rawModifierExpression, proficiencyBonusExpression], expressionName)
  } else {
  ...
  const modifierLabel = (
    (modifier === rawModifier)
      ...
      : <><ModifierLabel modifier={rawModifier} /> {arrow} <ModifierLabel modifier={modifier} /></>);
  ...
  return <tr>
    ...
    <td>
      {skillProficient ? 'Yes' : 'No'}
    </td>
    <td className='align-right'>
      {modifierLabel}
    </td>
    <td>
      <D20RollButton tag={skillName} expr={expression} />
    </td>
  </tr>
}
